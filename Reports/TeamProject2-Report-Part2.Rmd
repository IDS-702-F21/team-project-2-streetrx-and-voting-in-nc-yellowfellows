...
# Summary
This study investigates how various demographic groups vote in the 2020 general election. If the overall likelihood of voting in 2020 differs by county? What was the disparity in turnout rates between men and women for each political party? Finally, how did turnout rates differ by age group and party affiliation? We utilized the Hierarchical Logistic Regression model to answer the questions. As a first approach, we employed normal logistic regression to begin our modeling and a stepwise approach to variable selection. We generated a Null model as an intercept only and a Full model containing variables 'party cd,' 'race code,' 'ethnic code,"sex code,' and 'age.' All of the variables are retained in the resultant stepwise model. In compared to the baseline "Constitution party," the only negligible categories in the stepwise model are "Democrats" and "Republicans" in the 'party cd' variable. Only four counties have a non-significant random intercept, we may infer. Counties with increasing log odds ratios include Robeson, Cherokee, Anson, Pasquotank, Gates, Craven, Camden, Edgecomb, and Northhampton. We have two distinct degrees of sex engagement inside the gathering. Being a male member of the Green party increases the log odds ratio by 0.786 (2.19 odds ratio) as compared to being a female member of the Constitutional party. A Libertarian party member with an unspecified sex category is the second significant level. This level is linked to an increasing log odds ratio of 0.74. (2.10 odds ratio). The coefficients for the interaction between the party and age groups are more variable. As a result, regardless of party, the age group "26-40" is related with lower log odds when compared to the baseline age group "18-25" and Constitutional party category. The reverse impact is observed for the age group "Over 66," which is related with an increase in log odds ratio independent of party affiliation. The situation is more intriguing in the age group "41-65," which has one level Green party with declining odds ratio -0.09 (0.91 odds ratio), although this is not significant.

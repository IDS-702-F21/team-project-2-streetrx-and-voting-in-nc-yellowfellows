...
In order to answer the report questions, we use a hierarchical logistic regression model. As a first step, we model a regular logistic regression for our baseline model using the AIC stepwise approach for variable selection. Our Null model contains the intercept only and Full model includes all non-location predictor variables: `party_cd`, `race_code`, `ethnic_code`, `sex_code`, `age`. The resulting stepwise model keeps all the variables. The only insignificant categories in the stepwise model are "Democrats", "Republicans" categories in `party_cd` variable in comparison to the baseline "Constitution party" which gives us confidence to proceed further. As a next step, we include the`sex_code`:`party_cd` ,`age`:`party_cd` interactions to our stepwise model in order to address our questions of interest. As a final step, we add `county_desc` as a hierarchical level to our model. However, we experience several model convergence issues while fitting a hierarchical model due to having too many parameters to estimate for our given set of observations. Ultimately, improving our data pre-processing approach, such as aggregating not only the "History" data set, but also "Voters" as well as aggregating out both `vtd_abbrv` and `precinct_abbrv` in the two data sets helped us reach our final model.

*TODO: address lack of random slopes*
